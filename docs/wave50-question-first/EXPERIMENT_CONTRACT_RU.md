...
Traffic-quality cross-check: все `5` фактических query rows Wave46/48 соответствуют коммерческому интенту своих офферов; все клики получены через автотаргетинг. `35` ручных операторных фраз имеют `0` показов, поэтому их нельзя использовать как control. Это исключает очевидный query junk только для наблюдаемых пяти кликов, но из-за малого `N` не доказывает traffic quality и не разрешает менять targeting или Wave50 variable.
